...
import { Float, Text, Html, Sphere, MeshDistortMaterial, Line } from "@react-three/drei";
...
const Core = ({ logoUrl, large, reduced }) => {
  ...
  return (
    <group>
      ...
      <Html center distanceFactor={8}>
        <div className={`core-logo-container ${reduced ? 'reduced' : (large ? 'large' : '')}`}>
...
const Scene = ({ chartData, visible, large, showSegments, reduced }) => {
  ...
  useFrame((state) => {
    ...
    sceneRef.current.scale.lerp(new THREE.Vector3(targetScale, targetScale, targetScale), 0.05);
  });
  ...
  return (
    <group ref={sceneRef} scale={[0, 0, 0]}>
      ...
      {showSegments && chartData?.segments?.map((segment, i) => {
...
const HoveraCore = ({ chartData, visible, large, showSegments = true, reduced = false }) => {
